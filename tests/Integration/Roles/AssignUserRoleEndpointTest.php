...
describe('POST /users/{uuid}/assign-role', function (): void {
    it('rejects for unauthorized', function (): void {
        $this->postJson(
            getUrl(AppRouteNamesEnum::ASSIGN_USER_ROLE_ROUTE_NAME->value, ['user' => 'test']),
        )
    ...
    it('fails to assign a role with invalid payload', function (array $payload, string $errorMessage): void {
        $this->postJson(
            getUrl(AppRouteNamesEnum::ASSIGN_USER_ROLE_ROUTE_NAME->value, ['user' => $this->user]),
            data: $payload,
    ...
    it('returns not found response for non-existing user', function (): void {
        ...
        $this->postJson(
            getUrl(AppRouteNamesEnum::ASSIGN_USER_ROLE_ROUTE_NAME->value, ['user' => $invalidUuid]),
